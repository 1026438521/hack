...
public class PoolManager : Singleton<PoolManager> {
	...
	// {
	// 	GameObject item = Instantiate(ResourcesManager.Instance.getPrefabByName(name), new Vector3(0, 0, 0), Quaternion.Euler(0.0f, 0.0f, 0.0f)) as GameObject;
	...
	public GameObject getObjectByName(string name)
	{
		...
		if(dicList.ContainsKey(name) == false)
		{
			List<GameObject> objList = new List<GameObject>();
			dicList[name] = objList;
			item = Instantiate(ResourcesManager.Instance.getPrefabByName(name), new Vector3(0, 0, 0), Quaternion.Euler(0.0f, 0.0f, 0.0f)) as GameObject;
		}else
		{
		...
		{
			item.SetActive (true);
			SpawnPool pool = item.gameObject.AddComponent<SpawnPool> ();
			pool.reset(name);
		}
